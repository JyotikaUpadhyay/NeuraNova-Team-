...
export const AgentCommandCenter: React.FC<AgentCommandCenterProps> = ({
  ...
  const [logFilter, setLogFilter] = useState<
    'all' | 'tool_call' | 'thought' | 'recommendation'
  >('all');
  ...
  const agents = [
    ...
    },
    {
      ...
      status: 'COMPLETE',
      primaryMetric: `${(
        disruptionImpact.crudeDeficitBpd / 1_000_000
      ).toFixed(1)}M`,
    ...
    },
    {
      ...
      summary: `Top allocation: ${
        procurement.recommendedAllocations[0]?.supplier || 'US Gulf Coast'
      } (${procurement.recommendedAllocations[0]?.volumeBpd.toLocaleString()} bpd).`,
    ...
    },
    {
      ...
      summary: `${
        reserves.emergencyReleaseRecommended
          ? 'Emergency drawdown authorized'
          : 'Routine inventory monitoring'
      } at ${reserves.recommendedDrawdownRateMbblDay} MMbbl/d.`,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-[#111827] border border-slate-800 rounded-2xl p-4">
        <div>
          ...
          <p className="text-xs text-slate-400">
            5 specialized AI models executing concurrent threat assessment,
            commodity pricing, and supply rerouting
          </p>
        </div>
        ...
        {agents.map((agent) => {
          ...
          return (
            ...
            >
              <div>
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2.5">
                    <div
                      className={`p-2 rounded-xl bg-gradient-to-br ${agent.color} text-white shadow-md`}
                    >
                  ...
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full font-bold border ${agent.badgeColor}`}
                  >
                ...
                {isExpanded && (
                  <div className="space-y-1.5 pt-2 border-t border-slate-800 text-xs">
                    ...
                    {agent.reasoning.map((r, i) => (
                      <div
                        key={i}
                        className="text-[11px] text-slate-300 bg-slate-800/40 p-2 rounded-lg leading-snug"
                      >
                        • {r}
                      </div>
                    ))}
              ...
              >
                <span>
                  {isExpanded ? 'Collapse Reasoning' : 'View Chain of Thought'}
                </span>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-slate-800 pb-2">
          <div className="flex items-center gap-2 text-xs text-slate-300">
            ...
            <span className="font-bold text-white">
              Live Multi-Agent Event & Reasoning Stream
            </span>
          </div>
        ...
        <div className="h-44 overflow-y-auto space-y-1.5 text-xs pr-2">
          {filteredLogs.length === 0 ? (
            <div className="text-slate-500 text-xs italic py-4 text-center">
              Agent execution log buffer initialized. Trigger a scenario or ask
              AI Copilot to watch live reasoning.
            </div>
          ) : (
            filteredLogs.map((log) => (
              ...
              >
                ...
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase whitespace-nowrap ${
                    log.type === 'tool_call'
